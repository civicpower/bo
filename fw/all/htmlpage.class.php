...
class htmlpage{
    ...
    public function __construct($cache=true,$jquery=true,$tpldir=null){
		//module('module/smarty3/libs/SmartyBC.class.php');
		...
		$tpldir = realpath(dirname(__FILE__)."/../../html");
		$this->smarty->setTemplateDir($tpldir);
		$this->curdir = preg_replace("~\/$~","",$tpldir);
		...
		module(str_replace("//", "/", "/module/lang.php"));
		$this->doctype='<!DOCTYPE html>'.BN;
		...
		if($jquery){
			//$this->add_script('../../fw/module/jquery/jquery.js');
	...
	function set_append_file($file){
		$this->append=$this->html($file,true);
	}
	...
	function add_script($file,$include=false){
		...
		if(!in_array($file, $this->script_tab)){
			$possibles = array(
				$file,
				$file.'.js',
				//$this->curdir.'/html/js/'.$file,
				'js/'.$file,
				//$this->curdir.'/html/js/'.$file.'.js'
				'js/'.$file.'.js'
			);
			$done=false;
	...
	function add_style($file,$include=false){
	    $fin="";
	    if(preg_match("~(\?.*?)$~",$file,$match) && !preg_match("~^http~",$file)){
	        //printr2($match);exit();
            $fin=$match[1];
            $file = preg_replace("~\?.*?$~","",$file);
        }
		if(!in_array($file, $this->css_tab)){
			$possibles = array(
				$file,
				$file.'.css',
				//$this->curdir.'/html/css/'.$file,
				'css/'.$file,
				//$this->curdir.'/html/css/'.$file.'.css'
				'css/'.$file.'.css'
			);
			$done = false;
			foreach($possibles as $f){
			...
			if(!$done){
			    $this->css_tab[]=($include?'<include>':'').$file;
            }
		}
	}
	...
	function run_template(){
		...
		$this->smarty->assign('HTML_ENTETE',implode(BN,$this->entete_tab));
		$this->smarty->assign('HTML_ONLOAD',implode(BN,$this->onload));
		...
		$this->smarty->assign('HTML_APPEND',$this->append);
	...
	function html($f,$return=false){
		...
		if(!is_null($file)){
			...
			echo 'LE FICHIER '.$f.' N\'EXISTE PAS parmi '.print_r($possibles,true) ."\n";
			exit();
		}
	}
