...
// Load Composer's libs
    ...
    if ( ($lock = $pid->lock("cron_mail") ) <> FALSE ) {
        ...
        if(gget("code") != $_ENV['PASSWORD_CRON']){
            ...
            if ( isset($_ENV['DEBUG']) && ($_ENV['DEBUG']==TRUE) ) {
                error_log(date("Y-m-d h:i:sa", strtotime("now"))."   -> cron email enter ".$_SERVER['HTTP_HOST']."\n", 3, $_ENV['LOG_ERROR_CRON']);
            }
        }
        ...
        foreach ($list as $k => $v) {
            ...
            if (strlen($v['queue_numero']) > 0 && strlen($v['queue_sms_content']) > 0) {
                if ( isset($_ENV['DEBUG']) && ($_ENV['DEBUG']==TRUE) ) {
                    error_log(date("Y-m-d h:i:sa", strtotime("now"))."      -> sms to ".$v['queue_numero']."\n", 3, $_ENV['LOG_ERROR_CRON']);
                }
                // Debug Hakim
            ...
            if (strlen($v['queue_email']) > 0 && strlen($v['queue_email_html']) > 0) {
                if ( isset($_ENV['DEBUG']) && ($_ENV['DEBUG']==TRUE) ) {
                    error_log(date("Y-m-d h:i:sa", strtotime("now"))."      -> mail ".$v['queue_id']." to ".$v['queue_email']."\n", 3, $_ENV['LOG_ERROR_CRON']);
                }
                // Debug Hakim
                //if ($_SERVER["SERVER_NAME"] == "bo-ftp.civicpower.io" || $_SERVER["SERVER_NAME"] == "bo-dev.civicpower.io") {
        ...
        if ( isset($_ENV['DEBUG']) && ($_ENV['DEBUG']==TRUE) ) {
            error_log(date("Y-m-d h:i:sa", strtotime("now"))."|".$_SERVER['HTTP_HOST']."|   -> "." exit/already up\n", 3, $_ENV['LOG_ERROR_CRON']);
        }
    }
