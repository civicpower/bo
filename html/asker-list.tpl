        ...
        <div class="card card-gray-dark card-outline">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <a href="/asker" class="btn btn-lg bg-cp-purple text-white">
            ...
            <div class="table-responsive card-body p-0">
                <table class="table table-hover text-nowrap table-striped">
                    ...
                    {foreach from=$asker_list item=item key=key}
                        <tr>
                            <td><a href="/asker?asker_id={$item.asker_id|intval}" class="btn btn-sm"><i class="fa fa-edit"></i></a></td>
                            <td><a href="/asker?asker_id={$item.asker_id|intval}">{$item.asker_name}</a></td>
                            <td>{$item.nb_ballot}</td>
                            <td>
                                <button data-asker_id="{$item.asker_id|intval}" class="btn-remove btn btn-sm"><i class="fa fa-trash"></i></button>
                            </td>
                        </tr>
                    {/foreach}
                    </tbody>
